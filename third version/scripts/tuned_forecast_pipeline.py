...
arima_model = ARIMA(series, order=(5, 1, 5)).fit()
...
_, p_value = shapiro(df["residuals"].dropna()[:5000])
print(f"Residual Normality (Shapiro-Wilk p={p_value:.4f})")

# --- LSTM (nonlinear modeling on residuals) ---
scaler = MinMaxScaler()
...
plt.title("Tuned Hybrid ARIMA + LSTM Forecast")
plt.xlabel("Time (10-min intervals)")
plt.ylabel("Bytes")
plt.legend()
plt.grid(True)
